...
module InstallModel =
    ...
    let extractLibFolder path = extractPath "lib" path

    let extractBuildFolder path = extractPath "build" path
    ...
    // broken code
    let addLibReferences (libs: string seq) (installModel:InstallModel) : InstallModel =
    ...
    let createFromLibs frameworkRestrictions libs targetsFiles analyzerFiles frameworkAssemblyReferences = 
        emptyModel 
        |> addLibReferences libs 
        |> addTargetsFiles targetsFiles
        |> addAnalyzerFiles analyzerFiles 
        |> addFrameworkAssemblyReferences frameworkAssemblyReferences
        |> filterBlackList
...
type InstallModel with
    ...
    member this.AddLibReferences (libs, references) = InstallModel.addLibReferences libs this
   ...
   // member this.AddPackageFile (path, file, references) = InstallModel.addPackageFile path file references this    
    ...
    static member CreateFromLibs(frameworkRestrictions:FrameworkRestrictions, libs, targetsFiles, analyzerFiles,frameworkAssemblyReferences) = 
        InstallModel.createFromLibs  frameworkRestrictions libs targetsFiles analyzerFiles  frameworkAssemblyReferences